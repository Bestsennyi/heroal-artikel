...
A static, offline-capable Progressive Web App (PWA) — a warehouse/packing assistant for heroal logistics. The entire app is client-side: `index.html` holds all UI, CSS, and JavaScript; `sw.js` is the service worker; `manifest.json` is the PWA manifest; `icons/` holds icons; `links_heroal_artikel.txt` documents the published Google Sheets CSV export URLs used for data sync.
...
- **Data sync needs external network.** The "Sync" button (`triggerSync`) fetches live CSVs from Google Sheets (URLs in `links_heroal_artikel.txt` / `DEFAULT_URLS`). This requires outbound internet to `docs.google.com` and will fail offline — this does not indicate a broken environment.
